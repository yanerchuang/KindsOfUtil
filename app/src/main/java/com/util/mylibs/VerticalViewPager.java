...
public class VerticalViewPager extends ViewGroup {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        LogUtil.w("childCount " + childCount);
...
//          重点二、快速滑动时，如何判断当前显示的是第几个控件，并且再次包含边界判断（必须包含边界判断，因为前面的边界判断，只适用于低速滑动时）
                    ...
                    if ((getScrollY() >= childHeight * (currentPage - 1) + childHeight / offset && !shouZhiXiangXiaHuaDong)) {
                        //           手指向上滑动并且，滚动距离过了屏幕一半的距离
                        mScroller.startScroll(0, getScrollY(), 0, childHeight * (currentPage) - getScrollY());
                        currentPage++;

                    } else if ((getScrollY() < childHeight * (currentPage - 1) + childHeight / offset && !shouZhiXiangXiaHuaDong)) {
                        //           手指向上滑动并且，滚动距离没有过屏幕一半的距离
                        mScroller.startScroll(0, getScrollY(), 0, childHeight * (currentPage - 1) - getScrollY());

                    } else if ((getScrollY() <= childHeight * (currentPage - 2) + childHeight / (1 - offset) && shouZhiXiangXiaHuaDong)) {
